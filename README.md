...
- [Homebrew Bundle](https://docs.brew.sh/Brew-Bundle-and-Brewfile) for declarative macOS package and app installation
...
The macOS installer runs before Dotbot so a fresh machine gets the prerequisites first. It installs Xcode Command Line Tools if needed, bootstraps Homebrew, applies [`configs/macos/Brewfile`](configs/macos/Brewfile), enables Rosetta on Apple Silicon, installs a default Rust toolchain when Cargo is missing, and applies [`configs/macos/defaults.sh`](configs/macos/defaults.sh).
...
For a beginner-friendly first-boot checklist, see [`MACBOOK_AIR_SETUP.md`](MACBOOK_AIR_SETUP.md).
...
`qwen --thinking` for QwQ-32B; the server automatically loads the requested
model and keeps only one resident on the GPU. The shared Qwen config sets
`tools.approvalMode` to `auto-edit` so implementation requests write file
edits directly instead of returning code snippets. The `--thinking` wrapper
also appends a QwQ-specific system instruction that code-generation requests
must be applied to files with Qwen Code edit tools before answering.
