...
`scripts/install-qwen-code.sh`, links `~/.qwen/settings.json`, and provides
`qwen`, which opens an SSH tunnel to `weckerAA` by default and runs the local
Qwen Code CLI against the remote
`qwen3-coder-30b-a3b` service. Use `qwen --local` for `127.0.0.1`, or
