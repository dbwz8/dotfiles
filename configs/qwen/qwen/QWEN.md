...
- When asked to change files, apply the change with Qwen Code file-edit tools. Do not answer with terminal snippets, diffs, or replacement blocks instead of writing the files.
...
- If file-edit tools are unavailable or blocked, report that explicitly and stop. Do not fall back to showing snippets as if the requested edit was applied.
- After editing, inspect the changed files and report the files changed plus the focused validation command that was run.
