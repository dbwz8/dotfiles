...
write_auto_add_file() {
    ...
    load_file="$(mktemp "${TMPDIR:-/tmp}/aider-auto-add.XXXXXX")"
    {
        printf '%s\n' "# Generated by scripts/aider.sh; files are rooted at the launch directory."
        git -C "$PWD" ls-files --full-name -- \
            '*.md' \
            '*.rs' \
            '*.toml' \
            '*.go' \
...
if ! has_load_arg "$@"; then
    if auto_add_file="$(write_auto_add_file)"; then
        set -- --load "$auto_add_file" "$@"
    fi
fi
